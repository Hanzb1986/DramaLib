...
### 给你的规则变化
1. **投递入口：** 你用的共享 Input 是 `NovaShared/Drama/Input/`，不是老的 `NovaShared/Input/`
2. **成品出口：** 同步到 `NovaShared/Drama/OutPut/`
...
### 输出文件三步规则（你也要执行）
① 生成文件到自身 `workspace-drama/output/`
② 同步拷贝到共享 `NovaShared/Drama/OutPut/`
...
**每次启动必须：**
1. 查看 `NovaShared/.comm/to_Drama/` 目录是否有新消息
2. 有 `_URGENT` 前缀的优先处理
3. 读完移入 `NovaShared/.comm/archive/`
4. 需要回复就写到对方 `to_xxx/` 目录

> 详细协议见 `NovaShared/.comm/README.md` 或自己 01_BUSINESS_CONFIG.md §六
